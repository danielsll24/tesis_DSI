...
d <- read.csv(paste(dd, "quiLoc.csv", sep = ""), stringsAsFactors = FALSE)
head(d,1)
 
kk <- "/Volumes/DANIEL/comparaciones/"
k<-read.csv(paste(kk,"san.csv",sep=""),stringsAsFactors = F)
head(k,1)    
esc_loc<-merge(d,k,by="seccion")

gg <- "/Volumes/DANIEL/tesis_DSI/catalogo_secciones_ine/secciones/"
g <- read.csv(paste(gg,"qui_sec.csv",sep=""),stringsAsFactors=F)
head(g,1)
...
e <- read.dbf("/Volumes/DANIEL/Tesis/datos_geoelec/datos_geolectorales-25/a012869311d64a44b5a0d567cd20de04_geolectorales.dbf")
...
names(bastion_qui)[6]<-"SECCION"
esc_loc_pob <- merge(esc_loc_pob,bastion_qui,by="SECCION")
#seleccionar variables a considerar y quibiar nombres
head(esc_loc_pob,1)
db <- data.frame(esc_loc_pob$SECCION,esc_loc_pob$munn.x,esc_loc_pob$edon.x,
                 esc_loc_pob$disloc2015, esc_loc_pob$disloc2013, esc_loc_pob$POBTOT, esc_loc_pob$escenario1,
...
colnames(db)<- c("seccion","munn","edon","disloc2015","disloc2013","pobtot","esc1","esc3","urbana","rural","mixta","pan","pri","prd")
# seleccionar variables a considerar y quibiar nombres
#g <- data.frame(ff$SECCION, ff$munn, ff$edo, ff$edon, ff$disloc2013, ff$disloc2015,
#                ff$POBTOT)
#colnames(g) <- c("SECCION", "munn", "edo", "edon", "disloc2013", "disloc2015", "P_Total"

#merge(g,k,by="SECCION")
#head(g,1)
#colnames(g) <- c("SECCION","munn","edon","edo","disloc2015","disloc2013","esc1","esc2","esc3")
#p<-merge(g,e,by="SECCION")
#head(p,1)

#q <- data.frame(p$SECCION,p$munn,p$edon,p$edo,p$disloc2015,p$disloc2013,p$esc1,p$esc2,p$esc3,p$POBTOT)
#colnames(q) <- c("seccion","munn","edon","edo","disloc2015","disloc2013","esc1","esc2","esc3","P_Total")
E1 <- db$esc1 
#E3 <- db$esc3jge
#E3 <- db$esc3cg
E3 <-db$esc3
...
#dsi <- dsi_pob[,c("edon","disloc2015","disloc2013","esc1","esc3","dsi_pob","pobtot")]
#head(dsi)
...
write.csv(dsi, file = "/Volumes/DANIEL/tesis_DSI/archivos/dsi_SQ_E1_qui.csv", row.names = FALSE)
write.csv(db, file="/Volumes/DANIEL/tesis_DSI/archivos/db_SQ_E1_qui.csv", row.names = F)
...
write.csv(db, file="/Volumes/DANIEL/tesis_DSI/archivos/db_SQ_E3_qui.csv", row.names = F)
write.csv(dsi, file = "/Volumes/DANIEL/tesis_DSI/archivos/dsi_SQ_E3_qui.csv", row.names = FALSE)
...
write.csv(db, file="/Volumes/DANIEL/tesis_DSI/archivos/db_E1_E3_qui.csv", row.names = F)
write.csv(dsi, file = "/Volumes/DANIEL/tesis_DSI/archivos/dsi_E1_E3_qui.csv", row.names = FALSE)

#quitar los distritos donde c8 de las 3 tablas 
